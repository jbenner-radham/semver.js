...
export default function parsePrerelease(value = '') {
    ...
    if (errors.length > 1) {
        throw new AggregateError(errors, `Multiple TypeErrors were encountered when parsing the pre-release ${value}`);
    }

    return identifiers.map(identifier => (isIntLike(identifier) ? Number.parseInt(identifier) : identifier));
}
